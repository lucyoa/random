...
int main(int argc, char **argv) {
    ...
    fseek(f, 0, SEEK_END);
    size_t sz = ftell(f);   // size of the file
    fseek(f, 0, SEEK_SET);
    ...
    uint8_t *jit = (uint8_t *) mmap(0, 4096, PROT_READ|PROT_WRITE|PROT_EXEC, MAP_PRIVATE|MAP_ANONYMOUS, -1, 0);
    printf("%p\n", jit);

    while(bf[ctx.pc] != '\0') {
        switch(bf[ctx.pc]) {
            case '>': /*ctx.ptr++;*/ jit_me(&ctx, &bf[0]); continue;
            case '<': /*ctx.ptr--;*/ jit_me(&ctx, &bf[0]); continue;
